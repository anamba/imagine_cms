class Manage::CmsPagesController < Manage::ApplicationController
  ...
  def edit_page
    ...
    if @pg.send(@pg.new_record? ? :save : :save_without_revision)
      ...
      when 'preview'
        render js: "location.href = '#{"#{@pg.path.blank? ? '' : '/' + @pg.path}/version/#{@pg.published_version > 0 ? @pg.published_version : @pg.version}"}'"
      else
        flash[:notice] = 'Page saved.'
        session[:cms_pages_path] = @pg.path
        render js: "location.href = '#{url_for(action: 'index')}'"
      end
  ...
  def insert_page_object_config
    ...
    head :no_content and return unless params[:name]
    head :no_content and return unless params[:type] == 'page_list'
  end
